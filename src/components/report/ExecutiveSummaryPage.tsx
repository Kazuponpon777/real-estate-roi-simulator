    ...
    return (
        <div className="report-page flex flex-col">
            ...
            <div className="flex justify-between items-end mb-6 border-b-2 border-blue-600 pb-2 flex-shrink-0">
                <h2 className="text-2xl font-extrabold text-slate-800 tracking-tight">事業収支概要</h2>
                <p className="text-blue-400 text-[10px] uppercase tracking-widest">Page 02 — 全体概要</p>
            </div>

            {/* KPI Row */}
            <div className="grid grid-cols-4 gap-4 mb-6 flex-shrink-0">
                <div className="p-4 rounded-lg bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200">
                    <p className="text-[9px] font-bold text-blue-500 uppercase tracking-wider">表面利回り</p>
                    <p className="text-3xl font-extrabold text-blue-700 mt-1">{formatPercent(kpi.grossYield)}</p>
                    <p className="text-[9px] text-blue-400 mt-0.5">Gross Yield</p>
                </div>
                <div className="p-4 rounded-lg bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200">
                    <p className="text-[9px] font-bold text-emerald-500 uppercase tracking-wider">実質利回り (NOI)</p>
                    <p className="text-3xl font-extrabold text-emerald-600 mt-1">{formatPercent(kpi.netYield)}</p>
                    <p className="text-[9px] text-emerald-400 mt-0.5">Net Yield</p>
                </div>
                <div className="p-4 rounded-lg bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200">
                    <p className="text-[9px] font-bold text-amber-500 uppercase tracking-wider">税引前CF (年間平均)</p>
                    <p className="text-2xl font-bold text-amber-700 mt-1 leading-tight">{formatCurrency(kpi.beforeTaxCashFlow)}</p>
                    <p className="text-[9px] text-amber-400 mt-0.5">Average BTCF</p>
                </div>
                <div className="p-4 rounded-lg bg-gradient-to-br from-violet-50 to-violet-100 border border-violet-200">
                    <p className="text-[9px] font-bold text-violet-500 uppercase tracking-wider">総事業費</p>
                    <p className="text-2xl font-bold text-violet-700 mt-1 leading-tight">{formatCurrency(kpi.totalBudgetYen)}</p>
                    <p className="text-[9px] text-violet-400 mt-0.5">Total Investment</p>
                </div>
            </div>
                    ...
                    <table className="w-full text-xs text-left">
                        <thead>
                            <tr className="text-[9px] text-blue-400 uppercase border-b border-blue-100">
                                <th className="py-2 font-medium">借入先</th>
                                <th className="py-2 text-right font-medium">借入金額</th>
                                <th className="py-2 text-right font-medium">金利</th>
                                <th className="py-2 text-right font-medium">期間</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            ...
                            {data.funding.loans.length === 0 && (
                                <tr><td colSpan={4} className="py-4 text-center text-slate-400 italic text-[10px]">借入設定なし</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
